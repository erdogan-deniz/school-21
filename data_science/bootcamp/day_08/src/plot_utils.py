...
def draw_top_important_features(
    feat_names: ndarray | list, feat_importances: ndarray | list, top_size: int = 10
) -> None:
    """
...
def draw_clustering_model_parameter_optimization(
    ...
    for param_val in param_range:
        ...
        loc_clusterer_model: object = clusterer_model(
            **model_params,
        )
        silhouette_scores.append(
            silhouette_score(
                X,
                loc_clusterer_model.fit_predict(
                    X,
                ),
            ),
        )
    ...
    axs[0].set_title(
        f"Silhouette score of clustering model with differents {opt_param} " + "parameter values",
    )

    model_params[opt_param] = best_param_val
    clusterer_model: object = clusterer_model(
        **model_params,
    )
    clusters_samples: ndarray = clusterer_model.fit_predict(
        X,
    )
