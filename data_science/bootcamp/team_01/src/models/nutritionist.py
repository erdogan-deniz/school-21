...
class Nutritionist:
    ...
    def __init__(
        ...
        self.ingredients_nutrients_percentage: DataFrame = ingredients_nutrients_percentage
    ...
    def print_predicted_recipe_rating(pred_recipe_rating: float | None) -> None:
        ...
        try:
            ...
            print(
                f"It is {eval_val} idea to have a dish with that list of " + "ingredients.",
            )
    ...
    def predict_recipe_rating(self, ingredients: list[str] | None) -> float | None:
        """
    ...
    def print_ingredient_nutrients_facts(self, ingredient: str) -> None:
        ...
        try:
            ...
            ingredient = Nutritionist.get_correct_ingridient_name(
                ingredient,
            )
            ingredient_idx: int | None = self.ingredients_nutrients_percentage.index[
                self.ingredients_nutrients_percentage["name"] == ingredient
            ]
            if not ingredient_idx.empty:
                for nutrient_name, nutrient_val in self.ingredients_nutrients_percentage.iloc[
                    ingredient_idx[0]
                ][1:-1].items():
                    if nutrient_val > 0:
                        print(
    ...
    def print_ingredients_nutrients_facts(self, ingredients: list[str] | None) -> None:
        """
        Prints facts about the ingredient's nutrients.
        ...
        try:
            for ingredient in ingredients:
                self.print_ingredient_nutrients_facts(
                    ingredient,
                )
        except TypeError as type_err:
            raise TypeError(
    ...
    def find_top_three_similar_recipes(self, ingredients: list[str]) -> ndarray | None:
        """
    ...
    def print_top_three_similar_recipes(self, ingredients: list[str] | None) -> None:
        """
        ...
        try:
            for idx in self.find_top_three_similar_recipes(
                ingredients,
            ):
                print(
                    f"- {
        ...
        try:
            ...
            for ingredient_name in ingredients_names:
                ingredient_idx: int = self.ingredients_nutrients_percentage[
                    self.ingredients_nutrients_percentage["name"] == ingredient_name
                ].index[0]

                for nutrient_name, val in (
                    self.ingredients_nutrients_percentage.drop(
                        columns=[
                            "name",
                            "Unnamed: 0",
                        ],
            ...
            for nutrient_name, val in nutrients_data.items():
                print(
                    f"- {nutrient_name}: {val:.1f} %",
                )

            print(
                f"URL: {self.recipes_descriptions["url"].iloc[recipe_idx]}",
            )
        except IndexError as idx_err:
            raise IndexError(
                f"\nFile: {__file__}\n" + f"Message: {idx_err}.",
