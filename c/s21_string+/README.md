...
[![codecov](https://codecov.io/gh/erdogan-deniz/school-21/branch/main/graph/badge.svg?flag=c-s21_string%2B)](https://codecov.io/gh/erdogan-deniz/school-21?flags%5B0%5D=c-s21_string%2B)
[![License](https://img.shields.io/badge/license-MIT-green.svg)](../../LICENSE)

> *Own implementation of the C standard `string.h` library plus partial `sprintf`, `sscanf`, and a handful of C# `String`-class utilities — packaged as a static library `s21_string.a`.*
...
# Build and run the test suite (requires libcheck)
make test
./test
...
School 21 toolchain (`gcc`, `check`, `libsubunit-dev`, `lcov`, `gcovr`) — see
[`.github/workflows/c.yml`](../../.github/workflows/c.yml) for the canonical
install line.

## Demo

> **TODO** — terminal capture of `sprintf` formatter showcase is planned in the c/ Phase 2 demo slice.

## Documentation

- [string.h functions](#stringh-functions), [sprintf and sscanf](#sprintf-and-sscanf), and [special string processing functions](#special-string-processing-functions-from-the-string-class-in-c) sections below.
- API headers: `src/s21_string.h`.
- Coverage report: regenerated by `make gcov_report` (output in `src/test_html/`).
...
- Pattern: each `s21_*` function compared against the corresponding standard-library
  function for representative inputs; `sprintf` / `sscanf` exercised across the full
  matrix of flags × widths × precisions × length modifiers × specifiers.
- Required coverage: ≥ 80 % per function (gcov-checked).
- Run: `make test` (one-shot) or `make gcov_report` (with coverage).
