...
void miwidget::resizeGL(int w, int h) { glViewport(0, 0, w, h); }

//---------------------------

void miwidget::changeColor(double value) { BGcolor = value; }

void miwidget::changeLineColor(double value) { lineColor = value; }

void miwidget::changeLines(const QString& str) { lineType = str; }

void miwidget::lineSize(double value) { lineWidth = value; }
...
void miwidget::editMode(const QString& str) { mode = str; }

void miwidget::perspectiveMode(const QString& str) { perspective = str; }
...
void miwidget::paintGL() {
  if (flag_parsed_file) {
    ...
    if (mode == "Edges" && vertexType == "Point") {
      glDrawElements(GL_LINES, polygons.summ_of_polygons, GL_UNSIGNED_INT,
                     polygons.matrix_polygons);
    } else if (mode == "Vertexes" && vertexType == "Point") {
      glDrawElements(GL_POINTS, polygons.summ_of_polygons, GL_UNSIGNED_INT,
                     polygons.matrix_polygons);
    }
    if (vertexType == "Vertex") {
      glEnable(GL_POINT_SMOOTH);
    } else if (vertexType == "Circle") {
      glDrawElements(GL_CIRCULAR_TANGENT_ARC_TO_NV, polygons.summ_of_polygons,
                     GL_UNSIGNED_INT, polygons.matrix_polygons);
    } else if (vertexType == "Square") {
      glDrawElements(GL_QUADS, polygons.summ_of_polygons, GL_UNSIGNED_INT,
                     polygons.matrix_polygons);
    }
  }
