...
make install   # produces view/Smart_Calc(.app)
...
> **TODO** — short GIF of evaluating `2 / (3 + sin(x))^2` and plotting the resulting `f(x)` is planned in the c/ Phase 2 demo slice.

## Documentation

- [Polish & Reverse Polish notation](#polish-notation-and-reverse-polish-notation) and [Dijkstra's shunting-yard algorithm](#dijkstras-algorithm) sections below.
- Source layout:
  - `src/parser/` — tokeniser
  - `src/transform/` — infix → RPN (shunting-yard)
  - `src/calculate/` — RPN evaluator
  - `src/finance/` — credit & deposit business logic
  - `src/view/` — Qt UI
  - `src/tests/` — Check tests
- Doxygen API reference: planned in the c/ Phase 2 docs slice.

## Tests

- Framework: **Check** (LGPL, [libcheck/check](https://github.com/libcheck/check)).
- C-only `make test` covers parser / transform / calculate / validation / finance
  layers — no Qt dependency, runs in any Linux container.
