...
class MainWindow : public QMainWindow {
 ...
 private:
  ...
  // для гифок
  const int GifFps = 10,
            GifLength = 5;  ///< GIF parameters: 10 fps × 5 s = 50 frames.
 ...
 private slots:
  ...
  void
  on_pushButton_4_clicked();  ///< Rotate / move / scale (decoded via UI form).
  ...
  void on_horizontalSlider_4_valueChanged(
      int value);  ///< Background color slider.
  void on_comboBox_2_currentTextChanged(
      const QString &arg1);                            ///< Line-style combo.
  ...
  void on_comboBox_3_currentTextChanged(
      const QString &arg1);  ///< Vertex shape combo.
  void on_comboBox_4_currentTextChanged(
      const QString &arg1);  ///< Edit mode combo.
  void on_comboBox_5_currentTextChanged(
      const QString &arg1);        ///< Projection combo.
