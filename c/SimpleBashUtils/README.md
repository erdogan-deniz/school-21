...
[![codecov](https://codecov.io/gh/erdogan-deniz/school-21/branch/main/graph/badge.svg?flag=c-SimpleBashUtils)](https://codecov.io/gh/erdogan-deniz/school-21?flags%5B0%5D=c-SimpleBashUtils)
...
```bash
cd c/SimpleBashUtils/src

# Build cat clone
( cd cat && make )
./cat/s21_cat README.md

# Build grep clone
( cd grep && make )
./grep/s21_grep -i 'todo' ../../**/README.md

# Run the comparison test scripts (vs system bash utilities)
( cd cat && make test )
( cd grep && make test )
```

For a fully reproducible environment, build inside a Linux container with the
School 21 toolchain (`gcc`, `check`, `libsubunit-dev`, `lcov`) — see
[`.github/workflows/c.yml`](../../.github/workflows/c.yml) for the canonical
install line.

## Demo

> **TODO** — asciinema cast of `s21_cat -benst` and `s21_grep -in 'pattern' file` is planned in the c/ Phase 2 demo slice.

## Documentation

- [`cat` options](#cat-options) and [`grep` options](#grep-options) sections below.
- Source layout: `src/cat/`, `src/grep/` (each with its own Makefile).
- Doxygen API reference: planned in the c/ Phase 2 docs slice.

## Tests

- Strategy: **integration tests** — the test scripts run `s21_cat` / `s21_grep`
  against the same inputs as the host's `cat` / `grep` and compare outputs
  byte-for-byte.
- Run: `make test` inside `src/cat/` and `src/grep/`.
- Notes: requires a POSIX environment (Linux / macOS).
