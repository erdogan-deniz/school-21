...
def get_single_res(index: int, plugs, sockets, cables) -> str:  # Get one iter item
    try:
        return (
            f"plug {filter_and_convert(cables=cables, sockets=sockets)[index][0]}"
            + f" into {filter_and_convert(cables=cables, sockets=sockets)[index][1]}"
            + f" using {list(filter(check_item, plugs))[index]}"
        )
    except IndexError:
        return (
            f"weld {list(filter(check_item, cables))[index]}"
...
def fix_writing(plugs, sockets, cables) -> object:  # Main function
    # Solution:
    return [
        get_single_res(index=index, plugs=plugs, sockets=sockets, cables=cables)
        for index in range(len(filter_and_convert(cables=cables, sockets=sockets)))
    ]
...
class TestEx00(unittest.TestCase):
    ...
    result_two = ["plug cable2 into socket1 using plugZ", "plug cable1 into socket2 using plugY"]
    ...
    def tests_task(self):  # Tests from task
        self.assertEqual(
            fix_writing(plugs=self.plugs, sockets=self.sockets, cables=self.cables), self.result_one
        )
        self.assertEqual(
            fix_writing(plugs=self.plugs_two, sockets=self.sockets_two, cables=self.cables_two),
            self.result_two,
        )
    ...
    def tests_exception(self):  # Uncommon tests
        ...
        self.assertEqual(
            fix_writing(
                plugs=self.plugs_trash, sockets=self.sockets_trash, cables=self.cables_trash
            ),
            self.empty_result,
        )
    ...
    def tests_length(self):  # Different lengths tests
        self.assertEqual(
            fix_writing(plugs=[], sockets=self.sockets_length_one, cables=self.cables_length_one),
            self.length_result,
        )
        self.assertEqual(
            fix_writing(plugs=[], sockets=self.sockets_length_two, cables=self.cables_length_two),
            self.length_result,
        )
