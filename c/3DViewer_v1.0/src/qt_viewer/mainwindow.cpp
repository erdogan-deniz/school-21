...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(&fileBrowser, SIGNAL(nameChanged(const QString &)), this,
          SLOT(on_lineEdit_textChanged(const QString &)));

  connect(&fileBrowser, SIGNAL(objFile(obj_data)), this,
          SLOT(lineEdit_2_setText(obj_data)));
...
void MainWindow::on_pushButton_clicked() { fileBrowser.show(); }
...
void MainWindow::on_pushButton_4_clicked() {  // move up
  double value = 1;
  ogl->start_rotate(value, 'u');
}

void MainWindow::on_pushButton_3_clicked() {  // move left
  double value = -1;
  ogl->start_rotate(value, 'l');
}

void MainWindow::on_pushButton_2_clicked() {  // move right
  double value = 1;
  ogl->start_rotate(value, 'r');
}

void MainWindow::on_pushButton_5_clicked() {  // move down
  double value = -1;
...
void MainWindow::closeEvent(QCloseEvent *event) {
  QMessageBox::StandardButton resBtn = QMessageBox::question(
      this, "3d_viewer_0.1", tr("Save settings before exit?\n"),
...
void MainWindow::writeSettings() {
  ...
  settings->setValue("background color",
                     ui->horizontalSlider_4->sliderPosition());
...
void MainWindow::readSettings() {
  ...
  on_horizontalSlider_4_valueChanged(
      settings->value("backcround color").toDouble());
  ...
  on_comboBox_5_currentTextChanged(
      settings->value("ortho/perspective").toString());
}

void MainWindow::on_btn_settings_clicked() {
  QMessageBox::StandardButton resBtn = QMessageBox::question(
      this, "3d_viewer_0.1", tr("Load last saved settings?\n"),
