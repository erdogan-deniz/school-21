...
Window_Two ::Window_Two(QWidget* parent)
    : QWidget(parent), ui(new Ui ::Window_Two) {
  ui->setupUi(this);
...
Window_Two ::~Window_Two() { delete ui; }

void Window_Two ::on_line_total_credit_amount_textEdited() {
  ui->label_bigger_then->show();
}

void Window_Two ::on_line_term_textEdited() { ui->label_bigger_then2->show(); }
...
void Window_Two ::on_button_calculate_clicked() {
  ...
  long double total_credit_amount =
      s21_QString_to_long_double(ui->line_total_credit_amount->text());
  ...
  if (isnan(total_credit_amount) || total_credit_amount < 10 ||
...
void Window_Two ::s21_annuity_solution(long double total_amount, int term,
  ...
  if (total_amount && term && percents) {
    ...
    long double rate =
        pow((1 + s21_interest_rate_to_month_ratio(percents)), term);

    // Calculate:
    month_payment = total_amount * s21_interest_rate_to_month_ratio(percents) *
                    rate / (rate - 1);
    total_sum = month_payment * term;
    overpay = total_sum - total_amount;

    // Set lines value:
    ui->line_monthly_payments->setText(
        s21_long_double_to_QString(month_payment));
    ui->line_overpayment_on_credit->setText(
        s21_long_double_to_QString(overpay));
    ui->line_total_payment->setText(s21_long_double_to_QString(total_sum));
  }
}

void Window_Two ::s21_differtial_solution(long double total_amount,
                                          int month_duration,
                                          long double percents) {
  ...
  if (total_amount && month_duration && percents) {
    ...
    while (month_duration > 0) {
      payments = s21_push_payment(total_amount, month_duration, month, percents,
                                  payments);
      --month_duration;
      ++month;
    ...
    // Set answers:
    ui->line_monthly_payments->setText(
        s21_long_double_to_QString(total_sum / month));
  ...
  // Show payments:
  while (payments->prev) {
    ui->list_payments->addItem(
        "  debt:    " + s21_long_double_to_QString(payments->debt) +
        "          " +
