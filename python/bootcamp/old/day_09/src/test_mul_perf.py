...
def make_my_test(matrix_one: list, matrix_two: list):
    if (not isinstance(matrix_one, list)) or (
        not isinstance(matrix_two, list)
...
def make_comparison(first_matrix: list, second_matrix: list):
    if (not isinstance(first_matrix, list)) or (
        not isinstance(second_matrix, list)
    ):  # Incorrect type of argument case
        print("ERROR! INCORRECT ARGUMENT.")
    ...
    print(
        f"Speedup = {make_example_test(first_matrix, second_matrix) / make_my_test(first_matrix, second_matrix)}"
    )
