...
extern "C" long double s21_get_action_amount(int from, int to,
...
extern "C" s21_payments* s21_push_payment(long double sum, int month_duration,
                                          int month, long double percents,
                                          s21_payments* payments);
...
extern "C" s21_payments* s21_push_deposit(long double sum, int month,
                                          s21_payments* payments);
extern "C" s21_payments* s21_free_payments(s21_payments* payments);

Window_Three ::Window_Three(QWidget* parent)
    : QWidget(parent), ui(new Ui ::Window_Three) {
  ui->setupUi(this);
...
int Window_Three ::s21_QString_to_int(QString qstring) {
  ...
  return s21_is_int(string_convert_qstring)
             ? s21_string_to_int(string_convert_qstring)
             : -1;
}
...
void Window_Three ::on_button_calculate_clicked() {
  ...
  long double deposit_amount =
      s21_QString_to_long_double(ui->line_deposit_amount->text());
  int deposit_term = s21_QString_to_int(ui->line_deposit_term->text());
  long double interest_rate =
      s21_QString_to_long_double(ui->line_interest_rate->text());
  ...
  if (isnan(deposit_amount) || deposit_amount < 100 ||
  ...
  if (periodicity == -1 || periodicity < 1 || periodicity > 600 ||
      periodicity > deposit_term) {
    status = false;
  ...
  if (status) {
    s21_deposit_solution(deposit_amount, deposit_term, interest_rate, tax_rate,
                         periodicity, ui->checkbox_capitalization->isChecked(),
                         set_money, get_money);
  }
}

void Window_Three ::s21_deposit_solution(
    long double sum, int month_duration, long double percents,
    long double tax_percents, int ratio_payments_mounths, bool capital_percents,
  ...
  if (sum && month_duration && percents) {
    ...
    while ((month_duration / ratio_payments_mounths) > 0) {
      ...
      diff_pay += s21_get_action_amount(
          months_period, months_period + ratio_payments_mounths, set_money);
      diff_pay -= s21_get_action_amount(
          months_period, months_period + ratio_payments_mounths, get_money);

      months_period += ratio_payments_mounths;
      payment_sum += diff_pay;
      month_duration -= ratio_payments_mounths;
      base_profit = sum * s21_interest_rate_to_month_ratio(percents) *
                    ratio_payments_mounths;

      // Например: делится год по 4 месяца, то за 4 месяца молучим 1/3 от
      // начальной ставки
      base_profit += diff_pay;

      //            if (payment_sum < 0) {  // Check that we get not more money
      //            then have
      //                key = false;
      //                break;
      //            }
    ...
    ui->line_deposit_amount_by_the_end->setText(
        s21_long_double_to_QString(payment_sum));
  }
}
...
void Window_Three ::on_button_plus_clicked() {
  ...
  if (status) {
    ...
    ui->list_with->addItem(
        "  sum:    " + s21_long_double_to_QString(get_money->debt) +
        "        " + "month number:    " +
        s21_long_double_to_QString(get_money->month_number));
  }
}
...
void Window_Three ::on_button_plus2_clicked() {
  ...
  if (status) {
    ...
    ui->list_repl->addItem(
        "  sum:    " + s21_long_double_to_QString(set_money->debt) +
        "        " + "month number:    " +
        s21_long_double_to_QString(set_money->month_number));
  }
}
