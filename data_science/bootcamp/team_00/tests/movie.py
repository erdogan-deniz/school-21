...
class TestMovie:
    ...
    def test_movie_sequence_elements_data_types(self) -> None:
        ...
        assert isinstance(
            list(
                self.movies.get_release_years_movie_counts().keys(),
            )[0],
            str,
        )
        assert isinstance(
            list(
                self.movies.get_release_years_movie_counts().values(),
            )[0],
            int,
        )
        ...
        assert isinstance(
            list(
                self.movies.get_genres_counts().keys(),
            )[0],
            str,
        )
        assert isinstance(
            list(
                self.movies.get_genres_counts().values(),
            )[0],
            int,
        )
        ...
        assert isinstance(
            list(
                self.movies.get_most_genres_movies().keys(),
            )[0],
            str,
        )
        assert isinstance(
            list(
                self.movies.get_most_genres_movies().values(),
            )[0],
            int,
        )
        ...
        assert isinstance(
            list(
                self.movies.get_most_ratings_movies().keys(),
            )[0],
            str,
        )
        assert isinstance(
            list(
                self.movies.get_most_ratings_movies().values(),
            )[0],
            int,
        )
        ...
        assert isinstance(
            list(
                self.movies.get_top_ratings_metric_features().keys(),
            )[0],
            str,
        )
        assert isinstance(
            list(
                self.movies.get_top_ratings_metric_features().values(),
            )[0],
            float,
        )
    ...
    def test_movie_calculations(self) -> None:
        ...
        # Movie.get_genres_counts():
        assert self.movies.get_genres_counts() == self.correct_answers["get_genres_counts"]

        # Movie.get_most_genres_movies():
        assert (
            self.movies.get_most_genres_movies() == self.correct_answers["get_most_genres_movies"]
        )

        # Movie.get_most_ratings_movies():
        assert (
            self.movies.get_most_ratings_movies() == self.correct_answers["get_most_ratings_movies"]
        )
